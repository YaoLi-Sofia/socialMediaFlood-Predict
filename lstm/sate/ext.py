    ...
    def run(self, pre_data: ndarray, len_x, len_y, train_num, test_num, model_path, batch):
        ...
        # for epoch in range(1):
            ...
            for i, x in enumerate(train_x):
                ...
                loss_, _, _, _, _, _, _ = loss_func(output, train_y[i].unsqueeze(0))
            ...
            model.eval()
            with torch.no_grad():
                test_loss, min_p, max_p, mean_p, min_r, max_r, mean_r = [], [], [], [], [], [], []
                for i, x in enumerate(test_x):
                    pre_y = model(x.unsqueeze(0))
                    loss_, min_per, max_per, mean_per, min_rrm, max_rrm, mean_rrm = loss_func(pre_y, test_y[i].unsqueeze(0))
                    test_loss.append(loss_.item())
                    min_p.append(min_per)
                    max_p.append(max_per)
                    mean_p.append(mean_per)
                    min_r.append(min_rrm)
                    max_r.append(max_rrm)
                    mean_r.append(mean_rrm)
                ...
                if np.nanmin(min_p) >= 0.8:
                    if np.nanmin(max_p) >= 0.9:
                        if np.nanmax(min_r) < 0.2:
                            if np.nanmax(max_r) < 0.5:
                                break
                if np.nanmin(mean_p) > 0.8:
                    if np.nanmin(mean_r) < 0.3:
                        break
